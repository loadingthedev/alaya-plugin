(function (window, undefined) {
  ...
  function chatWindowShow(attachedText) {
    ...
    chatWindow.attachEvent("onWindowReady", function () {
      Asc.Editor.callMethod("ResizeWindow", [
        chatWindow.id,
        [400, 400],
        [400, 400],
        [0, 0],
  ...
  if (true) {
    ...
    button1.attachOnClick(async function () {
      ...
      else console.log("The text is correct, there are no errors in it.");
    });
  ...
  if (true) {
    ...
    button1.attachOnClick(function (data) {
      ...
      linkWindow.attachEvent("onGetLink", async function () {
        let link = data;
        if (!link) link = await Asc.Library.GetSelectedText();
        link = link.replace(/\n/g, "");
  ...
  buttonMainToolbar.text = "Linkinlegal";

  function getToolBarButtonIcons(icon) {
    return (
      "resources/icons/%theme-type%(light|dark)/big/" +
      icon +
      "%scale%(default).png"
    );
  }
  ...
  AI.ActionsLoad = function () {
    ...
    try {
      ...
      obj =
        AI.DEFAULT_SERVER_SETTINGS && AI.DEFAULT_SERVER_SETTINGS.actions
          ? AI.DEFAULT_SERVER_SETTINGS.actions
          : null;
    }

    if (obj) {
      for (let i in obj) {
        if (AI.Actions[i] && obj[i].model) AI.Actions[i].model = obj[i].model;
      }
      return true;
    }
    return false;
