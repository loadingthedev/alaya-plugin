...
window.Asc.plugin.init = async function () {
  initWithTranslate();
  clearChatState();
  const cookie = Asc.plugin.info.options.cookie;
  if (cookie) {
...
{\n\
    ...
    Api.AddCustomFunction(AI);\n\
      ...
      if (isUpdate)
        await Asc.Editor.callMethod("SetCustomFunctions", [
          JSON.stringify(oldCF),
        ]);
    }
  }
};
...
window.Asc.plugin.button = function (id, windowId) {
  ...
  if (settingsWindow && windowId === settingsWindow.id) {
    ...
    translateSettingsWindow &&
    windowId === translateSettingsWindow.id
  ) {
    if (id == 0) {
...
window.Asc.plugin.onThemeChanged = function (theme) {
  ...
  translateSettingsWindow &&
    translateSettingsWindow.command("onThemeChanged", theme);
  customProvidersWindow &&
    customProvidersWindow.command("onThemeChanged", theme);
...
function onOpenEditModal(data) {
  let variation = {
    ...
    description:
      data.type == "add"
        ? window.Asc.plugin.tr("Add AI Model")
        : window.Asc.plugin.tr("Edit AI Model"),
    isVisual: true,
    buttons: [
      { text: window.Asc.plugin.tr("OK"), primary: true },
      { text: window.Asc.plugin.tr("Cancel"), primary: false },
  ...
  if (!aiModelEditWindow) {
    ...
    aiModelEditWindow.attachEvent(
      "onOpenCustomProvidersModal",
      onOpenCustomProvidersModal
    );
  }
  ...
  if (!customProvidersWindow) {
    ...
    customProvidersWindow.attachEvent("onAddCustomProvider", function (item) {
      ...
      if (isError) {
        ...
        if (aiModelEditWindow)
          aiModelEditWindow.command("onProvidersUpdate", {
            providers: AI.serializeProviders(),
          });
      }
    });
    customProvidersWindow.attachEvent(
...
function onOpenSummarizationModal() {
  ...
  summarizationWindow.attachEvent("onInit", async function () {
    ...
    summarizationWindow &&
      summarizationWindow.command("onGetSelection", content);
  });
  ...
  summarizationWindow.attachEvent("onSummarize", async function (data) {
    switch (data.type) {
      case "review": {
        ...
        else await Asc.Library.InsertAsComment(data.data);
        break;
      }
      case "comment": {
