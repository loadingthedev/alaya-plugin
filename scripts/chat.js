(function (window, undefined) {
  ...
  let messagesList = {
    ...
    _renderItem: function (item) {
      ...
      const me = this;
      let $messageContent = $(
        '<div class="form-control message_content"></div>'
      );
      ...
      let htmlContent = c.render(removeSelectionTag(activeContent));
        ...
        if (false && item.attachedText) {
          $attachedWrapper = $(
            '<div class="message_content_attached_wrapper collapsed">' +
              ...
              getFormattedPathForIcon(
                "resources/icons/light/chevron-down.png"
              ) +
              '"/>' +
              "</div>" +
              "</div>"
          ...
          $attachedWrapper
            .find(".message_content_collapse_btn")
            .on("click", function () {
        ...
        if (item.error) {
          ...
          const $error = $(
            '<div class="message_content_error_title">' +
              '<img class="icon" draggable="false" src="' +
              getFormattedPathForIcon("resources/icons/light/error.png") +
              '" />' +
              "<div>" +
              errorObj.title +
              "</div>" +
              "</div>" +
              '<div class="message_content_error_desc">' +
          ...
          actionButtons.forEach(function (button, index) {
            let buttonEl = $(
              '<button class="action_button btn-text-default"></button>'
            );
            buttonEl.append(
              '<img class="icon" draggable="false" src="' +
                getFormattedPathForIcon(button.icon) +
                '"/>'
            );
          ...
          if (item.content.length > 1) {
            ...
            const $decrementBtn = $(
              '<button><img class="decrement icon" src="' +
                getFormattedPathForIcon(
                  "resources/icons/light/chevron-down.png"
                ) +
                '"/></button>'
            );
            item.activeContentIndex == 0
            ...
            const $incrementBtn = $(
              '<button><img class="increment icon" src="' +
                getFormattedPathForIcon(
                  "resources/icons/light/chevron-down.png"
                ) +
                '"/></button>'
            );
            item.activeContentIndex == item.content.length - 1
    ...
    add: function (item) {
      ...
      message.getActiveContent = function () {
        return message.role == "user"
          ? message.content
          : message.content[message.activeContentIndex];
      };
    ...
    pushContentForAssistant: function (messageIndex, content) {
      if (
        !this._list[messageIndex] ||
        this._list[messageIndex].role != "assistant"
      )
        return;
  ...
  let actionButtons = [
    {
      ...
      handler: function (message) {
        const messageIndex = messagesList.get().findIndex(function (item) {
          return item == message;
        });
        if (messageIndex > 0) {
    ...
    },
    {
      ...
      handler: function (message, content, htmlContent, plainText) {
        insertEngine("replace", plainText);
      },
    },
    {
      ...
      handler: function (message, content, htmlContent) {
        insertEngine("insert", htmlContent);
      },
    },
    {
      ...
      handler: function (message, content, htmlContent, plainText) {
        insertEngine("comment", plainText);
      },
    },
    {
      ...
      handler: function (message, content, htmlContent) {
        insertEngine("review", htmlContent);
      },
    },
  ];
  let welcomeButtons = [
    { text: "Legal blog post", prompt: "Legal blog post about" },
    { text: "Contract review", prompt: "Review this contract for" },
    {
      text: "Draft a demand letter",
      prompt: "Draft a demand letter regarding",
    },
    { text: "Legal research", prompt: "Legal research on" },
    { text: "Case brief", prompt: "Write a case brief for" },
    {
      text: "Client consultation notes",
      prompt: "Summarize these client consultation notes about",
    },
    { text: "Draft a legal memo", prompt: "Draft a legal memo regarding" },
    {
      text: "Prepare for deposition",
      prompt: "Prepare for a deposition about",
    },
    { text: "Analyze legal risk", prompt: "Analyze the legal risk of" },
  ];
  ...
  window.Asc.plugin.init = function () {
    ...
    document
      .getElementById("input_message_submit")
    ...
    document
      .getElementById("attached_text_close")
      .addEventListener("click", function () {
        attachedText.clear();
      });
  ...
  function renderWelcomeButtons() {
    welcomeButtons.forEach(function (button) {
      let addedEl = $(
        '<button class="welcome_button btn-text-default noselect">' +
          button.text +
          "</button>"
      );
  ...
  async function sendMessage(text) {
    ...
    const content = await window.Asc.Library.GetSelectedText();
    if (content) {
  ...
  function createTyping() {
    let chatEl = $("#chat");
    let messageEl = $(
      '<div id="loading" class="message" style="order: ' +
        messagesList.get().length +
        ';"></div>'
    );
    ...
    interval = setInterval(function () {
      ...
      spanMessageEl.text(
        window.Asc.plugin.tr("Thinking") + Array(countDots + 1).join(".")
      );
    }, 500);
  }
  ...
  function createLoader() {
    ...
    loader &&
      (loader.remove
        ? loader.remove()
        : $("#loader-container")[0].removeChild(loader));
    loader = showLoader(
      $("#loader-container")[0],
      window.Asc.plugin.tr("Loading...")
    );
  }
  ...
  window.Asc.plugin.onTranslate = function () {
    ...
    document
      .getElementById("input_message")
      .setAttribute("placeholder", window.Asc.plugin.tr("Ask AI anything"));
